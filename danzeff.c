...
///////////////////////////////////////////////////////////////////////////////////////////////////
// OSLIB Implementation by Sakya
///////////////////////////////////////////////////////////////////////////////////////////////////
#ifdef DANZEFF_OSLIB

OSL_IMAGE* keyBits[guiStringsSize];
int keyBitsSize = 0;
int moved_x = 0, moved_y = 0; // location that we are moved to
...
void danzeff_load()
{
	...
	for (a = 0; a < guiStringsSize; a++)
	{
		keyBits[a] = oslLoadImageFilePNG(guiStrings[a], OSL_IN_RAM | OSL_SWIZZLED, OSL_PF_8888);
		if (keyBits[a] == NULL)
		{
			//ERROR! out of memory.
			//free all previously created surfaces and set initialized to false
...
void danzeff_free()
{
	...
	for (a = 0; a < guiStringsSize; a++)
	{
		oslDeleteImage(keyBits[a]);
		keyBits[a] = NULL;
	}
	initialized = false;
}

void surface_draw_offset(OSL_IMAGE* pixels, int screenX, int screenY, int offsetX, int offsetY, int intWidth, int intHeight)
{
    OSL_IMAGE *imageTile = oslCreateImageTile(pixels, offsetX, offsetY, offsetX + intWidth, offsetY + intHeight);
    oslDrawImageXY(imageTile, moved_x + screenX, moved_y + screenY);
...
void danzeff_render()
{
	...
	//if center is selected then draw the whole thing opaque
	if (selected_x == 1 && selected_y == 1)
		oslDrawImageXY(keyBits[6*mode + shifted*3], moved_x, moved_y);
	else
		oslDrawImageXY(keyBits[6*mode + shifted*3 + 1], moved_x, moved_y);
